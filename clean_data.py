...
def create_fingerprint(df):
    ...
    df["key"] = df["key"].apply(lambda x: [stemmer.stem(word) for word in x])  # a la columna key le aplico una funcion apply
    # a cada elemento de la columna le aplica la función landa cuyo argumento es x la función es un conmprehension para cada
    #palabra aplquele es stemmer recibe la plabra y devuelve la raiz de la palabra 
